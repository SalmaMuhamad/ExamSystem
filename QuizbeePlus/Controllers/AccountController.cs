...
namespace QuizbeePlus.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult StudentRegister()
        {
            ...
            {
                //Ministries = MinistryService.Instance.GetAllMinistries(),
                //Categories = CategoryService.Instance.GetAllCategories(),
                //Halls = ControlPanelService.Instance.GetAllHalls()
            };
        ...
        public async Task<ActionResult> StudentRegister(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var OldUser = ControlPanelService.Instance.GetUserByID(model.NationalID);
                if (OldUser == null)
                {
                    var user = new QuizbeeUser
                    {
                        UserName = model.Name,
                        Email = model.Email,
                        NationalID = model.NationalID,
                        MobileNum = model.MobileNumber,
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> EmployeeRegister(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var OldUser = ControlPanelService.Instance.GetUserByID(model.Name);
                if (OldUser == null)
                {
                    var user = new QuizbeeUser
                    {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        //};
                        //ControlPanelService.Instance.AddCourse(Course);
                        //ControlPanelService.Instance.AddUserCourse(user, Course, model.HallID);

                        await SignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: false);

                        return RedirectToAction("ControlPanel", "DisplayUserSheet");
                    }
                    AddErrors(result);
                }
                else
